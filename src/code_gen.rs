...
impl<'a, 'b> Compiler for CodeGenerator<'a, 'b>
{
  fn compile_expr(&mut self, unquote: Unquote) -> RExpr {
    let state = bonsai::parse_expression(unquote.code.into_state());
  ...
  fn compile_block(&mut self, unquote: Unquote) -> RBlock {
    let state = bonsai::parse_program(unquote.code.into_state());
    match state.into_result() {
      Success(data) => {
...
impl<'a, 'b> CodeGenerator<'a, 'b>
{
  ...
  fn gen_placement_store(&self, unquote: &Unquote, store: bonsai::StorePlacement) -> RExpr {
    let store_name = unquote.text_to_ident[&store.store_name];
    match store.expr {
      bonsai::StoreExpression::Domain(range) => {
        let domain = self.gen_domain(unquote, range);
        quote_expr!(self.cx, $store_name.alloc($domain))
      }
      bonsai::StoreExpression::Constraint(constraint) => {
  ...
  fn gen_domain(&self, unquote: &Unquote, range: bonsai::Range) -> RExpr {
  ...
  fn gen_arith_expr(&self, unquote: &Unquote, arith_expr: bonsai::AExpr) -> RExpr {
    use grammar::bonsai::ArithExpr::*;
    match *arith_expr {
  ...
  fn gen_statements(&self, unquote: &Unquote, statements: Vec<bonsai::Statement>) -> RBlock {
    let rust_stmt = statements.into_iter()
      .map(|stmt| self.gen_statement(unquote, stmt)).collect();
    self.cx.block(unquote.span, rust_stmt)
  }

  fn gen_statement(&self, unquote: &Unquote, statement: bonsai::Statement) -> rust::Stmt {
    use grammar::bonsai::Statement::*;
    match statement {
  ...
  fn gen_constraint(&self, unquote: &Unquote, constraint: bonsai::Constraint) -> RExpr {
    use grammar::bonsai::Constraint::*;
    match constraint {
      Binary(constraint) => {
  ...
  fn gen_binary_constraint(&self, unquote: &Unquote, constraint: bonsai::BinaryConstraint) -> RExpr {
    use grammar::bonsai::RelationalOp::*;
  ...
  fn gen_var_view(&self, unquote: &Unquote, arith_expr: bonsai::AExpr) -> RExpr {
    use grammar::bonsai::ArithExpr::*;
    match *arith_expr {
      Variable(var) => self.gen_ident(unquote, var),
      Number(n) => {
  ...
  fn gen_bin_arith_expr(&self, unquote: &Unquote, op: bonsai::BinArithOp,
    x: bonsai::AExpr, y: bonsai::AExpr) -> RExpr
  {
    use grammar::bonsai::BinArithOp::*;
  ...
  fn gen_nary_constraint(&self, unquote: &Unquote, constraint: bonsai::NaryConstraint) -> RExpr {
  ...
  fn gen_indexed_expr(&self, unquote: &Unquote, var: String, index: bonsai::AExpr) -> RExpr {
